...
class WordPressAPI(commands.Cog):
	...
	@app_commands.describe(
		website_login="Wordpress Login Name",
		application_password="Wordpress Application Password"
	)
	async def link(self, ctx, website_login: str, application_password: str):
		author = ctx.user.display_name
		nameid = ctx.user.id
		server = ctx.guild.name
		
		wordpress_id = website_login
		wordpress_secret = application_password
