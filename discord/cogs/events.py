...
from renfield_sql import get_member, check_is_auth, get_log_channel, update_event, get_nice, add_member, renfield_sql, get_bot_setting
...
class Events(commands.Cog):
	...
	async def signin(self, ctx):
		...
		memberinfo = get_member(nameid, server)
		member_id = memberinfo["member_id"]
		if member_id == 0:
			memberinfo = add_member(nameid, author, server)
			member_id = memberinfo["member_id"]
