...
class Settings(commands.Cog):
	...
	@app_commands.choices(setting_name=[
		...
		app_commands.Choice(name="Set OAuth consumer key", value="consumer_key"),
		app_commands.Choice(name="Set OAuth consumer secret", value="consumer_secret"),
		...
		app_commands.Choice(name="Set webhook URL for Harker integration", value="harker-webhook"),
	...
	async def _config(self, ctx, setting_name: str, setting_value: str = ""):
		...
		#available settings
		settings = ['list-all', 'admin_role', 'voice-activity', 'cubby-hole', 'wordpress_site', 'harker-webhook', 'consumer_key', 'consumer_secret', 'polly_voice', 'event_location', 'event_start', 'event_end', 'event_desc']
		...
		if setting_name == 'list-all':
			...
			if setting_value == "":
				...
				if setting_name == "wordpress_site":
					# remove any trailing /
