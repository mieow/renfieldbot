...
def save_link(nameid, wordpress_id, server, token, secret, status):
	mydb = renfield_sql()
	mycursor = mydb.connect()
	ok = 1
	...
	if count == 0:
		# insert
		try:
			sql = "INSERT INTO wp_link (server, name, wordpress_id, token, secret, auth_status) VALUES (%s, %s, %s, %s, %s, %s)"
		...
		# update
		try:
			sql = "UPDATE wp_link SET wordpress_id = %s, token = %s, secret = %s, auth_status = %s WHERE name = %s and server = %s"				
...
def get_link(nameid, server):
	...
	# get the current level of the setting
	try:
		sql = "select wordpress_id, token, secret, auth_status from wp_link where name = %s and server = %s"
		val = ("{}".format(nameid), server)
		mycursor.execute(sql, val)
		result = mycursor.fetchall()
		if len(result) > 0:
			info["wordpress_id"] = result[0][0]
			info["token"] = result[0][1]
			info["secret"] = result[0][2]
