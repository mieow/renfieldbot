...
@bot.command(name='list', help='list game events: .list [all | <event>]')
async def list(ctx, action: str='next'):
	...
	try:
		if action == 'all':
			...
			events = session.query(Event).filter(Event.server == guild).order_by(Event.date).all()
			rows = [[e.name, e.date, 0] for e in events]
		elif action == 'next':
			# list next event
			headers = ['Name', 'Date']
			events = session.query(Event).filter(Event.server == guild, Event.date >= datetoday).order_by(Event.date).all()
			rows = [[events[0].name, events[0].date]]
		else:
			# list users who attended
			headers = ['Name', 'Date']
			events = session.query(Event).filter(Event.server == guild, Event.date >= datetoday, Event.name == action).all()
